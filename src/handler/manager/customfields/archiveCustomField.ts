...
export const archiveCustomField = async (
  ...
  try {
    ...
    if (
      ...
      res.status(403).send({
        ...
        displayError:
          "You need to be a scouting lead to archive custom fields",
      });
      return;
    }
    ...
    if (field.teamNumber !== req.user.teamNumber) {
      res.status(403).send({
        ...
        displayError: "Not authorized to archive this custom field",
      });
      return;
    }
    ...
    res.status(200).send("done archiving custom field");
  } catch (error) {
    console.error(error);
