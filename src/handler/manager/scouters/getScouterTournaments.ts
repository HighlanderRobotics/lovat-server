  ...
  try {
      ...
      if (req.query.skip != undefined) {
        if (req.query.take != undefined) {
          ...
          await prismaClient.tournament.findMany({
            ...
            where: {
              OR: [
                { key: { contains: params.data.filter } },
    ...
    if (req.query.filter != undefined) {
      const tempRows = await prismaClient.tournament.findMany({
        where: {
          OR: [
            { key: { contains: req.query.filter as string } },
    ...
    if (!params.success) {
      res
        .status(400)
        .send({ error: params, displayError: "Team code is not correct." });
      return;
    }
    ...
    if (!teamRow) {
      res.status(400).send({
        error: `x-team-code sent was ${params.data.code} and is not affilated with a team.`,
        displayError: "Team code is not correct.",
      });
      return;
    }
