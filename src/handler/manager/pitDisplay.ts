...
import { matchPredictionLogic } from "@/src/handler/analysis/alliancePredictions/matchPredictionLogic.js";
import { MatchType } from "@prisma/client";
import { MatchEnumToAbrivation } from "@/src/handler/manager/managerConstants.js";
...
export const pitDisplay = async (
  ...
  try {
    ...
    await axios
      .get(
        `https://www.thebluealliance.com/api/v3/event/${params.data.tournamentKey}`,
        { headers },
      )
      .then(async (response) => {
    ...
    await axios
      .get(
        `https://www.thebluealliance.com/api/v3/event/${params.data.tournamentKey}/rankings`,
        { headers },
      )
      .then(async (response) => {
        ...
        const findTeamIndex = mappedData.findIndex(
    ...
    if (nowPlaying) {
      ...
      matchesData.nowPlaying = await matchFormat(
        ...
        nowPlaying.matchNumber,
        nowPlaying.matchType,
      );
      matchesData.next = await matchFormat(
        params.data.tournamentKey,
      ...
      if (matchesData.next !== null) {
        ...
        prevMatchAllRows.filter((match) => prevMatchAllKeys.push(match.key));
        for (const match of allPrevMatches) {
          for (let i = 0; i < 6; i++) {
            prevMatchAllKeys.push(
              match.tournamentKey +
                "_" +
                MatchEnumToAbrivation[match.matchType] +
                match.matchNumber +
                "_" +
                i,
            );
        ...
        if (nextTeamMatch) {
          matchesData.teamNext = await matchFormat(
            params.data.tournamentKey,
            nextTeamMatch.matchNumber,
            nextTeamMatch.matchType,
          );
        }
        ...
        if (
          ...
          if (nowPlaying.matchType !== teamPrevMatch.matchType) {
            data.teamMatchTimeline = {
              matchCount:
                maxQualifierRow.matchNumber -
                teamPrevMatch.matchNumber +
                nextTeamMatch.matchNumber,
              currentMatchCount:
                maxQualifierRow.matchNumber -
                teamPrevMatch.matchNumber +
            ...
            data.teamMatchTimeline = {
              matchCount:
                ...
                nextTeamMatch.matchNumber,
              currentMatchCount:
                nowPlaying.matchNumber - teamPrevMatch.matchNumber + 1,
            };
          }
        } else {
          data.teamMatchTimeline = {
            matchCount: nextTeamMatch.matchNumber - teamPrevMatch.matchNumber,
            currentMatchCount:
              nowPlaying.matchNumber - teamPrevMatch.matchNumber + 1,
          };
        }
...
async function matchFormat(
  ...
  try {
    ...
    const blueTeams = [
      fullLatestedScouted[3].teamNumber,
      fullLatestedScouted[4].teamNumber,
      fullLatestedScouted[5].teamNumber,
    ];
    const redTeams = [
      fullLatestedScouted[0].teamNumber,
      fullLatestedScouted[1].teamNumber,
      fullLatestedScouted[2].teamNumber,
    ];
    const key =
      tournamentKey + "_" + MatchEnumToAbrivation[matchType] + matchNumber;
    //CHANGE
