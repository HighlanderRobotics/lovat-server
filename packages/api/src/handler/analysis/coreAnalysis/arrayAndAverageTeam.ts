...
export const arrayAndAverageTeam = async (
  ...
  average: number;
  timeLine: { match: string; dataPoint: number }[];
}> => {
  try {
    const params = z
      .object({
    ...
    for (let i = 0; i < tournamentAverages.length; i++) {
      if (i === 0) {
        ...
        const scoutedMatchesAtMostRecentTournament = timeLineArray.filter(
          (item) => item.match.startsWith(recentTournament)
        ).length;
        ...
        const weightOnRecent =
          ...
          (1 -
            1 /
              ((multiplerBaseAnalysis * scoutedMatchesAtMostRecentTournament) /
