...
export const picklistShell = async (
  ...
  try {
    ...
    const allTeamData: {
      average: number;
      teamAverages: Map<number, number>;
      timeLine: number[];
    }[] = [];
    if (params.data.tournamentKey) {
      ...
      for (const metric of picklistSliders) {
        if (params.data[metric] || params.data.flags.includes(metric)) {
          const currData = await picklistArrayAndAverageAllTeamTournament(
            req.user,
            metric,
            includedTeamNumbers
          );
    ...
    for (let i = 0; i < allTeamData.length; i++) {
      ...
      if (
        currData.average !== null &&
        !isNaN(currData.average) &&
        currData.average !== undefined &&
        currData.timeLine.length >= 2 &&
        ss.standardDeviation(currData.timeLine)
      ) {
    ...
    for (const teams of teamChunks) {
      if (teams.length > 0) {
        teamBreakdowns.push(
          createWorker(teams, metricAllTeamMaps, allTeamAvgSTD, params, req)
        );
      }
    }
    const dataArr = [];
      ...
      for (const currTeamData of data) {
        for (const currZscores of currTeamData) {
          if (!isNaN(currZscores.zScore)) {
            const temp = {
              team: currZscores.team,
              result: currZscores.zScore,
              breakdown: currZscores.adjusted,
              unweighted: currZscores.unadjusted,
              flags: currZscores.flags,
            };
            dataArr.push(temp);
          }
        }
...
function createWorker(teams, metricAllTeamMaps, allTeamAvgSTD, params, req) {
  return new Promise((resolve, reject) => {
    ...
    const worker = new Worker(
      "././dist/handler/analysis/picklist/zScoreTeam.js",
      { type: "module" }
    );
